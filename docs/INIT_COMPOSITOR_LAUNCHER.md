...
7. **Launcher gráfico en wawa** (Spotlight-like): vueltas 1–10 shipped en Fase 58 — overlay modal `Alt+P`, navegación Alt+J/K + Enter, ratón completo (hover + clic + clic-fuera-cancela), búsqueda por texto en vivo, contador "N/M" en el título, **match jerárquico v5** (prefijo > substring > subsecuencia, selección sticky), **highlight v6** de los chars matcheados, **scroll vertical v7** con auto-track de la selección, **atajos numéricos v8** (`Alt+1..9` lanza la fila visible N), **instalación viva v9** (`kernel::instalar_app(EntradaApp)`) y **polling automático v10** del manifiesto del disco cada ~6 s desde `tarea_compositor`. Paridad funcional con Spotlight ya conseguida; el cableado de origen (mudanza vía re-ancla, cronista vía nuevas entradas) está vivo end-to-end.
