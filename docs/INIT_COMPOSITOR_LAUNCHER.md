...
7. **Launcher gráfico en wawa** (Spotlight-like): vueltas 1–5 shipped en Fase 58 — overlay modal `Alt+P`, navegación Alt+J/K + Enter, ratón completo (hover + clic + clic-fuera-cancela), búsqueda por texto en vivo, contador "N/M" en el título, y **match jerárquico v5** (prefijo > substring > subsecuencia, selección sticky a través de refiltrados). Falta: scroll vertical si el catálogo crece más allá de las 16 filas visibles (hoy son 12 apps, sobra), instalación en vivo de apps desde el grafo (Akasha/cronista). Estimado pendiente: 1 sesión para llegar a paridad funcional con Spotlight.
...
- **2026-05-27 (wawa Fase 58)**: launcher gráfico vueltas 1–5 — `Alt+P` abre overlay modal con teclado + ratón modales, búsqueda por texto en vivo, Escape/Backspace soportados sin Alt. Cola `PARTOS_POR_INDICE` paralela a la legacy de rotación. Mirror atómico `LAUNCHER_ABIERTO` para que el IRQ del teclado decida sin tomar locks. Contador "N/M" en la barra de título (v4). **Match jerárquico v5**: prefijo > substring > subsecuencia (estilo Spotlight, `plm` → `pluma`), con selección *sticky* a través de refiltrados — `evaluar_match` puro reemplazó al `contiene_ascii_ci` lineal de v3.
