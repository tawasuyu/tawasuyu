...
7. **Launcher gráfico en wawa** (Spotlight-like): vueltas 1–6 shipped en Fase 58 — overlay modal `Alt+P`, navegación Alt+J/K + Enter, ratón completo (hover + clic + clic-fuera-cancela), búsqueda por texto en vivo, contador "N/M" en el título, **match jerárquico v5** (prefijo > substring > subsecuencia, selección sticky a través de refiltrados), y **highlight v6** de los chars matcheados en cada fila (Spotlight-classic, máscara `u64` por nombre). Falta: scroll vertical si el catálogo crece más allá de las 16 filas visibles (hoy son 12 apps, sobra), instalación en vivo de apps desde el grafo (Akasha/cronista). Estimado pendiente: ~1 sesión para llegar a paridad funcional con Spotlight (lo grande es la instalación en vivo).
