//! `gioser-launcher` — el launcher real de gioser sobre el motor único.
//!
//! No es el demo: descubre las apps de verdad (`app_bus::AppRegistry`,
//! sembrando `~/.config/gioser/apps/` la primera vez), carga la
//! `Surface` de `~/.config/gioser/launcher.toml` (o el escritorio por
...
/// `~/.config/gioser/launcher.toml` (respeta `XDG_CONFIG_HOME`).
fn surface_config_path() -> Option<std::path::PathBuf> {
    let base = std::env::var_os("XDG_CONFIG_HOME")
        .map(std::path::PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|h| std::path::PathBuf::from(h).join(".config")))?;
    Some(base.join("gioser").join("launcher.toml"))
}
