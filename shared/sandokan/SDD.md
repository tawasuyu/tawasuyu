...
- **App dedicada `sandokan-monitor-llimphi`** ✅ 2026-06-01 (frontend Fase 3, variante "app propia"): monitor de procesos sobre Llimphi, dos mundos en una fachada. **Linux**: tarjetas vivas por unidad (punto de estado por color, **sparkline de CPU** vía `paint_with`, memoria/hilos/restarts), polling a 1 Hz por `observe()` sobre el Engine que elige `auto_default()`; seleccionar una tarjeta revela detener (grace 3 s) / matar (grace 0), ambos por `Engine::stop` — el monitor controla por el **mismo contrato** que observa. **Wawa**: censo host-side de las apps WASM instaladas (assets del kernel), con nota explícita de que el censo del executor en vivo es Fase 4. `SANDOKAN_MONITOR_SEED=1` siembra unidades reales (`IsolationLevel::None`) para demo sin init. Binario: `sandokan-monitor`.
