...
> Flat C-friendly ABI for [tinkuy](../README.md).

Surfaces `tinkuy-core`'s `World + Grid3D` through a handle-based, `extern "C"` API designed to cross WASM and FFI boundaries without exposing Rust generics. Wraps everything behind an opaque `TkSim` handle and returns `i32` status codes.
...
Status codes: `TK_OK = 0`, `TK_ERR_NULL = -1`, `TK_ERR_INVALID = -2`, `TK_ERR_OOM = -3`.

## Use sites

- `03_ukupacha/wawa/apps/tinkuy` — re-exports this crate as a cdylib (`pub use tinkuy_abi::*;`) → the WASM blob loaded by `wawa-kernel`.
- `wawa-kernel/src/tinkuy.rs` — resolves these symbols as `wasmi::TypedFunc`.
