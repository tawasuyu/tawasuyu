...
/// Botón de impresión: manda la hoja al navegador del SO.
fn print_button(theme: &Theme) -> View<Msg> {
    View::new(Style {
        size: Size {
            width: length(180.0),
            height: length(28.0),
    ...
    .text_aligned("Imprimir hoja…".to_string(), 12.0, theme.fg_text, Alignment::Center)
    .on_click(Msg::PrintSheet)
}

/// La hoja imprimible en pantalla: una previsualización en papel (cabecera
/// de la carta + tabla de aspectos) con el botón para mandarla al diálogo
/// de impresión del navegador. Se ve mejor en «Modo impresión» (tema B/N),
/// pero funciona en cualquier tema. Top-aligned y de ancho de hoja para
/// que el botón quede siempre visible aunque la tabla sea larga.
pub(crate) fn print_sheet(model: &Model, theme: &Theme) -> View<Msg> {
    ...
    .text_aligned("Carta natal".to_string(), 18.0, theme.fg_text, Alignment::Start);

    let sheet = View::new(Style {
        flex_direction: FlexDirection::Column,
        size: Size {
            width: length(540.0),
            height: Dimension::auto(),
    ...
    .children(vec![
        print_button(theme),
        titulo,
        tile_carta(model, theme),
        section_label("Aspectos".to_string(), theme),
        tile_aspectos(&model.render, theme),
    ]);

    // Contenedor de área completa, top-aligned: anula el centrado vertical
    // del `graphic_area` para que el botón no se vaya fuera de pantalla
    // cuando la tabla de aspectos es alta.
    View::new(Style {
        flex_direction: FlexDirection::Column,
        size: Size {
            width: percent(1.0_f32),
