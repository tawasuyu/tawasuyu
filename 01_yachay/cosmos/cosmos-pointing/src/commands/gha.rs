...
impl Command for Gha {
    ...
    fn execute(&self, session: &mut Session, args: &[&str]) -> Result<CommandOutput> {
        ...
        if let Some(path) = args.first() {
            ...
            terminal_output(&dx_vs_ha, &dd_vs_ha)
        }
    }
}
...
mod tests {
    ...
    fn no_fit_returns_error() {
        let mut session = Session::new();
        let result = Gha.execute(&mut session, &[]);
    ...
    fn empty_observations_returns_message() {
        ...
        let result = Gha.execute(&mut session, &[]).unwrap();
        match result {
    ...
    fn terminal_shows_both_dx_and_ddec() {
        ...
        match result {
            CommandOutput::Text(s) => {
                assert!(s.contains("dX vs Hour Angle"));
                assert!(s.contains("dDec vs Hour Angle"));
            }
            _ => panic!("expected Text output"),
        }
    }
    ...
    fn svg_writes_two_files() {
        ...
        let path = dir.join("gha_test.svg");
        let path_str = path.to_str().unwrap();
        let result = Gha.execute(&mut session, &[path_str]).unwrap();

        let dx_path = dir.join("gha_test_dx.svg");
        let dd_path = dir.join("gha_test_dd.svg");

        match &result {
            CommandOutput::Text(s) => {
                assert!(s.contains("Written to"));
                assert!(s.contains("_dx.svg"));
                assert!(s.contains("_dd.svg"));
            }
            _ => panic!("expected Text output"),
        }
