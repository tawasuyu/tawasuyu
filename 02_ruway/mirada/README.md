...
- **Wallpaper** del escritorio (`config.ron` → `wallpaper_path`): PNG/JPEG/WebP escalado a la salida, compuesto al fondo (backend DRM). **Multi-monitor**: `outputs: [(name: "HDMI-A-1", wallpaper_path: "…", wallpaper_fit: "fill")]` permite un fondo distinto por conector; lo que no se indique cae al global. Hot-reload aplica el cambio sin reiniciar.
