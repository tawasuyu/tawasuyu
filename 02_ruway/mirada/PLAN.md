...
| **Capabilities por ventana** | 🔨 EN CURSO (1ª rebanada hecha) | **Clipboard (`zwlr_data_control`) gateado por ejecutable.** El snoop de portapapeles ya no se concede a todos: el Cuerpo **no anuncia el global** a los clientes denegados (frontera física, no tabla eludible). Identidad = ejecutable real vía `SO_PEERCRED → /proc/<pid>/exe` (verdad del kernel, no el `app_id` falsificable; los clientes data_control ni tienen superficie). Política `Permisos{clipboard_denylist}` autorada en el Cerebro (`mirada-brain/src/permisos.rs`, RON en `~/.config/mirada/caps.ron`), empujada al Cuerpo por `BrainCommand::SetCapabilities` (espejo de `SetDecorations`) y aplicada en el filtro `Fn(&Client)->bool` de `DataControlState::new`. Postura: **permitir por defecto**. Pendiente: screencopy (hay que **implementarlo** primero — hoy no existe) + más globals (foreign-toplevel, virtual-input) repitiendo el patrón. |
