...
impl LoudnessMeter {
    pub fn new(sample_rate: u32, channels: u16) -> Self {
        ...
        let weights = (0..channels.max(1))
            .map(|i| channel_weight(i, channels.max(1)))
            .collect();
        // 100 ms por sub-bloque; al menos 1 sample para no dividir por cero.
        let samples_per_subblock = ((sample_rate as usize) / 10).max(1);
        LoudnessMeter {
            sample_rate,
            channels: channels.max(1),
            weights,
            filters: k_weighting(sample_rate as f64),
            states: vec![(BiquadState::default(), BiquadState::default()); ch],
            samples_per_subblock,
            cur_sumsq: vec![0.0; ch],
            cur_count: 0,
            subblocks: Vec::new(),
            block_z: Vec::new(),
        }
    ...
    /// Alimenta el medidor con `buf` (samples `f32` intercalados por canal).
...
/// alterarlo (tap pasivo, igual molde que el probe del visor). Útil para medir
/// en vivo mientras se reproduce y después fijar la ganancia.
...
impl<S> LoudnessProbe<S> {
    pub fn new(inner: S, sample_rate: u32, channels: u16) -> Self {
        LoudnessProbe {
            inner,
    ...
    pub fn gain_to_target_db(&self, target_lufs: f32) -> Option<f32> {
        self.meter.gain_to_target_db(target_lufs)
    }
}
...
impl<S: AudioSource> AudioSource for LoudnessProbe<S> {
    fn fill(&mut self, buf: &mut [f32], sample_rate: u32, channels: u16) {
        ...
        // El medidor se construyó con un rate/canales fijos; si el sink usa
        // otros, no medimos (evita coeficientes desfasados).
        if sample_rate == self.meter.sample_rate() && channels == self.meter.channels() {
            self.meter.push(buf);
        }
    }
}
...
mod tests {
    ...
    fn probe_no_altera_y_mide() {
        ...
        let mut probe = LoudnessProbe::new(Sine { phase: 0.0 }, 48_000, 2);
        ...
        assert!(probe.integrated_lufs().is_some());
    }
}
