//! media-source-vorbis — decoder Vorbis (vía symphonia) como [`AudioSource`].
//!
//! Vorbis es el **lossy clásico libre de patentes** del tier nativo:
//! el tercero del trío de audio abierto junto a Opus (lossy moderno) y
//! FLAC (lossless). Decoder + demuxer Ogg de symphonia, puro-Rust, sin
//! C ni patentes — compila a WASM y corre en wawa.
//!
//! Misma forma que [`media_source_flac::FlacSource`] /
//! [`media_source_mp3::Mp3Source`]: decodea el archivo entero a `f32`
//! intercalado al construir, y el callback reproduce en loop con
//! resampleo lineal cuando el sample rate del sink difiere. Trade-off
//! explícito: RAM = duración · sample_rate · channels · 4 bytes. Para
//! audios largos habría que hacer streaming por bloques.
//!
//! Implementa [`media_core::Seekable`] sobre el cursor de frames del
//! source — el wrapper del app mueve la posición sin saber el formato.
...
impl std::fmt::Display for VorbisError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Empty => write!(f, "Vorbis sin samples"),
        }
    }
}
...
impl From<std::io::Error> for VorbisError {
    fn from(e: std::io::Error) -> Self {
        Self::Io(e)
    }
}
impl From<SymphError> for VorbisError {
    fn from(e: SymphError) -> Self {
        Self::Symphonia(e)
    }
}

/// Reproductor Vorbis en loop. Misma semántica de `cursor` y resampleo
/// lineal que `FlacSource` / `Mp3Source` / `WavSource`.
pub struct VorbisSource {
    samples: Vec<f32>,
    src_channels: u16,
    src_sample_rate: u32,
    cursor: f64,
    /// Multiplicador de velocidad — mismo varispeed que el resto (no hay
    /// time-stretching: cambia pitch). Clampeado en `set_speed`.
    speed: f32,
    /// Loop infinito al pasar el final (default true).
    looped: bool,
    finished: bool,
}

impl VorbisSource {
    pub fn from_path(path: impl AsRef<Path>) -> Result<Self, VorbisError> {
        ...
        let track = format
            ...
            .ok_or(VorbisError::NoAudioTrack)?;
        ...
        loop {
            let packet = match format.next_packet() {
                ...
                Err(e) => return Err(VorbisError::Symphonia(e)),
            };
...
impl Seekable for VorbisSource {
...
/// intercalado. Vorbis entrega F32 normalmente; cubrimos todos los
/// formatos de symphonia por completitud.
...
mod tests {
    ...
    const FIXTURE: &str = concat!(
        ...
        "/tests/fixtures/tone_440_stereo.ogg"
    );

    #[test]
    fn decodes_real_fixture() {
        let src = VorbisSource::from_path(FIXTURE).expect("decode Vorbis fixture");
        assert_eq!(src.source_channels(), 2);
        assert_eq!(src.source_sample_rate(), 48_000);
        let dur = src.duration_seconds();
        assert!((dur - 1.0).abs() < 0.1, "duración ~1s, fue {dur}");
    }

    #[test]
    fn fill_produces_signal() {
        let mut src = VorbisSource::from_path(FIXTURE).expect("decode Vorbis fixture");
        let mut buf = vec![0f32; 48_000 * 2];
        src.fill(&mut buf, 48_000, 2);
        let energy: f32 = buf.iter().map(|s| s * s).sum::<f32>() / buf.len() as f32;
        assert!(energy > 1e-3, "señal con energía, fue {energy}");
    }

    #[test]
    fn seek_moves_cursor() {
        let mut src = VorbisSource::from_path(FIXTURE).expect("decode Vorbis fixture");
        src.seek_to(Duration::from_millis(500));
        let pos = src.position();
        assert!(
            (pos.as_secs_f32() - 0.5).abs() < 0.02,
            "seek a 0.5s, fue {:?}",
            pos
        );
    }
