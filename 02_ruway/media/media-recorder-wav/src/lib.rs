//! media-recorder-wav — captura el stream de audio a un archivo WAV.
//!
...
///
/// [`ProbedAudioSource`]: media_core::ProbedAudioSource
...
mod tests {
    ...
    fn round_trip_short_recording() {
        ...
        let path = dir.join("media_recorder_wav_test.wav");
    ...
    fn start_without_format_fails() {
        let rec = WavRecorder::new();
        let path = std::env::temp_dir().join("media_recorder_wav_no_fmt.wav");
...
/// Conveniencia para nombrar archivos `media-rec-YYYYMMDD-HHMMSS.wav`.
...
pub fn default_recording_path(dir: impl AsRef<Path>) -> PathBuf {
    ...
    let secs = SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let name = format!("media-rec-{secs}.wav");
