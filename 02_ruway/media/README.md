...
| `media-source-capture`| **Captura en vivo** (lado INPUT): cámara v4l2 · **pantalla X11** (→ `FrameSource`) + **micrófono cpal** (→ `AudioSource`). Núcleos agnósticos: `LiveSource`/`LiveSink` (slot latest-frame para video) y `AudioLiveSource`/`AudioLiveSink` (ring para audio) + conversión pura de pixel-formats (YUYV/MJPEG/RGB/BGR + 32-bit X11 BGRX/XRGB). Tres backends opt-in: `CameraSource` (feature `camera`), `ScreenSource` (feature `screen`, x11rb puro-Rust) y `MicSource` (feature `mic`, cpal, pide 48 kHz Opus-able). Todos alimentan `media-recorder-webm` → grabá cámara/pantalla **+ audio** a `.webm` AV1+Opus nativo (ejemplos `grabar_pantalla` / `grabar_pantalla_audio`). Ver su README. |
