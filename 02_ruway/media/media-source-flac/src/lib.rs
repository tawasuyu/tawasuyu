...
//!
//! Misma forma que [`media_source_mp3::Mp3Source`]: decodea el archivo
//! entero a `f32` intercalado al construir, y el callback reproduce en
//! loop con resampleo lineal cuando el sample rate del sink difiere del
//! del FLAC. Trade-off explícito: RAM = duración · sample_rate ·
//! channels · 4 bytes. Para audios largos habría que hacer streaming
//! por bloques.
//!
...
impl std::fmt::Display for FlacError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Empty => write!(f, "FLAC sin samples"),
        }
    }
}
...
impl From<SymphError> for FlacError {
    fn from(e: SymphError) -> Self {
        Self::Symphonia(e)
    }
}

/// Reproductor FLAC en loop. Misma semántica de `cursor` y resampleo
/// lineal que `Mp3Source` / `WavSource`.
pub struct FlacSource {
    samples: Vec<f32>,
    src_channels: u16,
    src_sample_rate: u32,
    cursor: f64,
    /// Multiplicador de velocidad — mismo modo varispeed que MP3/WAV (no
...
impl FlacSource {
    pub fn from_path(path: impl AsRef<Path>) -> Result<Self, FlacError> {
        ...
        loop {
            let packet = match format.next_packet() {
                ...
                Err(e) => return Err(FlacError::Symphonia(e)),
            };
...
/// intercalado. FLAC entrega normalmente S16/S24/S32 según el bit-depth
/// del archivo; cubrimos todos los formatos de symphonia por completitud.
...
mod tests {
    ...
    const FIXTURE: &str = concat!(
        ...
        "/tests/fixtures/tone_440_stereo.flac"
    );

    #[test]
    fn decodes_real_fixture() {
        let src = FlacSource::from_path(FIXTURE).expect("decode FLAC fixture");
        // tono 440 Hz, 1 s, estéreo, 48 kHz.
        assert_eq!(src.source_channels(), 2);
        assert_eq!(src.source_sample_rate(), 48_000);
        let dur = src.duration_seconds();
        assert!((dur - 1.0).abs() < 0.05, "duración ~1s, fue {dur}");
    }

    #[test]
    fn fill_produces_signal() {
        let mut src = FlacSource::from_path(FIXTURE).expect("decode FLAC fixture");
        let mut buf = vec![0f32; 48_000 * 2];
        src.fill(&mut buf, 48_000, 2);
        // El tono debe tener energía no trivial.
