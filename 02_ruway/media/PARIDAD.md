...
- **M1 (sync A/V) ✅**: política `AvSync` (kernel) + `FrameSource` lleva PTS + **wiring en `media-app`**: el video tickea por reloj de pared (el source ya respeta el fps del archivo vía su acumulador) y `AvSync::plan` **descarta** los frames que llegan tarde respecto del reloj de audio; el dup es implícito (sin frame nuevo se retiene la textura). El reloj de audio se lee con `try_lock` (jamás bloquea el hilo de UI). **NOTA**: una primera versión esclavizaba el `dt` del video al delta del reloj de audio — deadlockeaba al arranque (el pipe de ffmpeg: el video alimenta al audio, así que regular el video por el audio se muerde la cola; además el paint bloqueaba en el lock del Playlist que cpal tiene mientras lee el pipe). Corregido: decode por wall-clock, audio sólo para drop.
