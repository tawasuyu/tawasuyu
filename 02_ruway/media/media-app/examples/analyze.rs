...
//!
//! ```text
//! cargo run -p media-app --example analyze --release -- audio.wav
//! cargo run -p media-app --example analyze --release -- track.mp3 out.png
//! ```
//!
...
use media_core::{AudioSource, Waterfall};
...
fn main() {
    ...
    let input = match args.first() {
        ...
        None => {
            eprintln!(
                "uso: cargo run -p media-app --example analyze --release -- <audio> [out.png]"
            );
            std::process::exit(2);
        }
    };
