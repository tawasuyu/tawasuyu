//! multimedia-source-ffmpeg — puente al binario `ffmpeg` para decodificar
...
//! dominio (`shared/foreign-*`, `multimedia-source-*`) trata ffmpeg
...
use multimedia_core::{AudioSource, FrameSource, Seekable};
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Unsupported => write!(
                f,
                "multimedia-source-ffmpeg requiere Unix (pre_exec + dup2)"
            ),
        }
    }
}
