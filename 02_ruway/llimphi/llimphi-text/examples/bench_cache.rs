...
fn pintar_frame(ts: &mut Typesetter, frame: usize, estatico: bool) {
    ...
    for label in CHROME {
        let _ = ts.layout(label, 13.0, None, Alignment::Start, 1.2, false, None, 400.0, false, false, 0.0, 0.0, false);
    }
    let _ = ts.layout(PARRAFO, 15.0, Some(420.0), Alignment::Start, 1.4, false, None, 400.0, false, false, 0.0, 0.0, false);
    ...
    let _ = ts.layout(&dinamico, 13.0, None, Alignment::Start, 1.2, false, None, 400.0, false, false, 0.0, 0.0, false);
}
...
fn corrida_sin_cache() {
    ...
    for f in 1..=FRAMES {
        for t in frame_texts(f) {
            let _ = ts.layout(&t, 13.0, Some(420.0), Alignment::Start, 1.2, false, None, 400.0, false, false, 0.0, 0.0, false);
        }
    }
    let dt = t0.elapsed();
    println!(
