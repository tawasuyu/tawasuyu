...
//! genera y maneja el *contenido* de un mundo voxel (world-gen, y a futuro
//! bloques/biomas/streaming) sobre el motor 3D **general** [`llimphi_3d`], que
//! es quien renderiza ([`llimphi_3d::Scene3d`] + [`llimphi_3d::VoxelRenderer`]).
//!
//! Es una rama de dependencias pensada como **librería reusable por cualquier
//! juego voxel** de la suite: `app → llimphi-voxel → llimphi-3d → wgpu`. El
//! motor (cámara, depth compartido, ray-march, mallas) no sabe nada de juegos;
//! acá vive lo que sí (terreno, materiales, dinámica de mundo).
//!
//! Hoy expone [`terrain`]; el resto (chunks, streaming, bloques tipados) crece
