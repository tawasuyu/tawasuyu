...
- **OAuth2 (`XOAUTH2`)** — for Gmail/Outlook, which closed IMAP/SMTP to
  ...
  for a fresh token on every SMTP send and, if an IMAP op fails because **the
  session died** — the server rejected the expired token (auth) or the connection
  dropped — reconnects with a new token and retries once. Logical failures (missing
  mailbox, parse) surface as-is, so it never reconnects needlessly. So you
