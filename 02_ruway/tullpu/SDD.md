...
9. **Comparativos por luminosidad** ✓ — `ModoFusion::ColorMasOscuro` (Photoshop "Darker Color") y `ColorMasClaro` ("Lighter Color"). No factorizan por canal: la decisión es por píxel completo — `Lum(src) ⋚ Lum(dst)` selecciona el triple ganador y se devuelve entero. `mezclar_canal` los cortocircuita junto a los HSL antes del despacho per-channel; convención de empate: gana `dst`. `foreign-psd::mapear_blend` cablea los discriminantes 7 (DARKER_COLOR) y 12 (LIGHTER_COLOR) directo. App: el ciclador `blnd` ahora recorre 22 modos. **Único residuo degradado tras esta fase: `Dissolve`** (PRNG por píxel — requiere semilla estable y queda fuera del modelo determinista actual de `mezclar_canal`). Tests cubren: rojo (Lum=0.30) gana sobre verde (Lum=0.59) en ColorMasOscuro; el triple completo viaja (no per-canal — un canal alto del fondo no sobrevive si el triple pierde); empate elige dst. Pendiente: máscaras de capa PSD (el crate `psd` 0.3 no expone `UserSuppliedLayerMask` públicamente — bloqueado upstream), jerarquías de grupos PSD, `Dissolve`.
