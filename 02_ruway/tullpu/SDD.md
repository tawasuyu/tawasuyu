...
15. **Rasterización de grupos PSD non-Normal** ✓ — `foreign-psd` se refactorizó con un árbol explícito `NodoPsd::{Capa{idx}, Grupo{id, hijos}}` reconstruido en `construir_arbol(layers, grupos)` (orden entre hermanos inferido del menor índice de hoja en su descendencia, recursivo y memoizado). `procesar_nodo` recorre en post-order: grupos `Normal`/`PassThrough` se aplanan (mismo comportamiento de Fase 14: prefix path + multiplicar opacidad + AND visible); grupos con blend distinto se **rasterizan** vía `rasterizar_grupo` — un sub-`Lienzo` con sus capas se compone con `tullpu_render::componer` a un único Rgba8 hasheado, y el grupo entero queda como una sola `Capa` raster con el blend, opacidad y visibilidad propios del grupo (las del grupo se aplican al resultado, las internas ya están dentro del raster). `VistaBuffers<'a>` adapta `&HashMap<Hash, Vec<u8>>` al trait `FuenteBuffers` sin copias. Recursión segura: los subgrupos non-Normal se rasterizan antes que su padre (post-order), así un Multiply > Screen > capa devuelve un Screen ya colapsado al composite del Multiply. `foreign-psd` agrega dep en `tullpu-render` (sólo dev antes). Renombre: `grupos_con_blend_propio` → `grupos_rasterizados: Vec<(nombre, blend_debug)>`, semántica positiva — son los grupos que efectivamente se compusieron. App: header muestra `(N rasterizados)` en lugar de `(N grupos no-Normal)`. Tests nuevos (3, total 14 verde): `rasterizar_grupo_multiplicar_da_color_esperado` (gris × blanco = gris, tolerancia ±1 por canal), `rasterizar_grupo_capa_invisible_no_aporta`, `rasterizar_grupo_dedup_misma_composicion` (mismo hash entre llamadas idénticas). Sin fixture PSD con grupos non-Normal en el corpus upstream → la validación end-to-end del flujo PSD→raster queda para un fixture sintético propio cuando alguien lo necesite. Pendiente: file picker en la app, drag & drop, export a PSD (post-MVP), máscaras de capa PSD (bloqueado upstream).
