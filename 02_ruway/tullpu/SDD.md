...
14. **Jerarquías de grupos PSD aplanadas** ✓ — el bridge `foreign-psd` recorre `psd.groups()` y, para cada capa, resuelve la cadena de grupos por encima vía `parent_id` (memoizada por `id` en un `HashMap<u32, CadenaGrupo>` — un grupo se resuelve a lo sumo una vez). Las propiedades del grupo se **propagan a la capa hoja**: el `nombre` lleva la ruta jerárquica (`"raíz/sub/hoja"`), la `opacidad` es el producto (clamped a `[0,1]`), la `visibilidad` es AND lógico. El blend de la capa se preserva tal cual; si algún grupo de la cadena tiene blend distinto de `Normal`/`PassThrough`, se anota en `InformeImportacion.grupos_con_blend_propio` — modelarlo fielmente requeriría rasterizar el grupo a un buffer intermedio (post-MVP). `InformeImportacion` ganó dos campos: `grupos_detectados: usize` y `grupos_con_blend_propio: Vec<(ruta, blend)>`. La app extiende el header con `· N grupos` cuando hay grupos y suma anotaciones `(X blend→Normal, Y grupos no-Normal)` si hubo divergencias. Tres fixtures nuevos del corpus upstream (`group-one-layer`, `groups-nested`, `groups-siblings`) cubren: 1 grupo / 1 capa, grupos anidados, hermanos. Tests: path jerárquico contiene `/` sin doble-slash, anidados producen ≥3 segmentos, hermanos tienen raíces distintas, opacidades en `[0,1]`, PSD plano no introduce `/` en nombres (regresión). Pendiente: file picker en la app (la carga y el destino del export siguen siendo path-based), drag & drop, export a PSD (post-MVP), máscaras de capa PSD (bloqueado upstream), blend de grupo no-Normal (requiere rasterización intermedia).
