...
fn resolve_tab(entry: &config::TabEntry) -> Option<Instance> {
    resolve_instance(
        &entry.id,
        entry.source.clone(),
        entry.label.clone(),
...
fn render_main_full(inst: &Instance, theme: &Theme) -> View<Msg> {
...
fn render_tabs_with_monitors(model: &Model, theme: &Theme) -> View<Msg> {
...
fn tabs_toolbar(model: &Model, theme: &Theme) -> View<Msg> {
...
fn tab_content(model: &Model, theme: &Theme) -> View<Msg> {
    let Some(inst) = model.tabs.get(model.active_tab) else {
