...
pub struct Config {
    ...
    /// IA + semántica — opt-in, configurable desde wawa-panel. Vacío = LLM por
    /// env (`from_env`) y búsqueda semántica apagada.
    #[serde(default)]
    pub ai: AiConfig,
}

/// `[ai]` del shumarc: el LLM (instrumento de `:?`/`:explica`/`:resume`) y la
/// búsqueda semántica (`:buscar`). Tipos planos (sin depender de pluma-llm ni
/// rimay-verbo) para mantener este crate liviano; el shell los traduce a sus
/// fachadas. `""`/`0` = "sin fijar, usar el default".
///
/// ```toml
/// [ai.llm]
/// backend = "anthropic"        # "" = auto (from_env); anthropic/gemini/deepseek/cohere/ollama/mock
/// model = "claude-sonnet-4-6"  # "" = default del backend
/// # api_key = "..."            # "" = leer del env (recomendado)
///
/// [ai.semantic]
/// enabled = true
/// # socket = "/run/user/1000/verbo.sock"  # "" = socket por defecto del daemon
/// dim = 384                     # dimensión del fallback mock si no hay daemon
/// ```
#[derive(Debug, Clone, PartialEq, Eq, Default, Serialize, Deserialize)]
pub struct AiConfig {
    #[serde(default)]
...
/// Selección de backend LLM. `backend` vacío = resolver por env (`from_env`).
#[derive(Debug, Clone, PartialEq, Eq, Default, Serialize, Deserialize)]
...
impl LlmSettings {
    /// `true` si fija un backend explícito (si no, el shell cae a `from_env`).
    pub fn is_set(&self) -> bool {
        !self.backend.trim().is_empty()
    }
}

/// Ajustes de la búsqueda semántica (`:buscar`).
#[derive(Debug, Clone, PartialEq, Eq, Default, Serialize, Deserialize)]
pub struct SemanticSettings {
    /// Habilita `:buscar`. Apagado por defecto.
    #[serde(default)]
    pub enabled: bool,
