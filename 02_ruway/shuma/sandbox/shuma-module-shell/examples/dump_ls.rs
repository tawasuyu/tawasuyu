//! Reproducción headless de un `ls` largo en estado estable (viewport ya
//! medido): comprueba si el output se acota y baja-alinea, o si aplasta el
//! input. `cargo run -p shuma-module-shell --example dump_ls -- [out.png]`
...
fn main() {
    let out = std::env::args().nth(1).unwrap_or_else(|| "ls.png".to_string());
    let theme = llimphi_theme::Theme::default();
    let mut state = shuma_module_shell::State::new(shuma_module::Source::Local);
    state.input.set_text("cat archivo_03.rs");

    use shuma_module_shell::OutputLine;
    let block = 1u64;
    let mut push = |state: &mut shuma_module_shell::State, mut l: OutputLine| {
        l.block = block;
        state.output.push(l);
    };
    ...
    // Estado estable: el painter ya midió el viewport en un frame previo.
    *state.out_viewport_h.lock().unwrap() = 480.0;
    ...
    eprintln!("dump_ls: escrito {out} ({W}x{H})");
}
