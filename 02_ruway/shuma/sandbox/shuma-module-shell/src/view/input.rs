...
/// Fila completa del input: la línea (flex) + chips de opciones a la
/// derecha — `env N` (lista las variables aprendidas; `:env` las setea)
/// y `persist` (alterna la persistencia de la sesión, encendido = accent).
/// Los chips quedan abajo, alineados a la línea aunque el input crezca
/// multilínea.
pub(crate) fn input_row_view<HostMsg: Clone + 'static>(
    state: &State,
    theme: &Theme,
    lift: impl Fn(Msg) -> HostMsg + Clone + Send + Sync + 'static,
) -> View<HostMsg> {
    let input_holder = View::new(Style {
        flex_grow: 1.0,
        size: Size {
    ...
    .children(vec![shell_input_view(state, theme, lift.clone())]);

    let chip = |label: String, activo: bool, msg: Msg| {
        let (fill, fg) = if activo {
        ...
        .on_click(lift(msg))
        .text_aligned(label, 10.0, fg, Alignment::Start)
        .mono()
    };
    let n_env = state.config.env.len();
    let env_chip = chip(
        if n_env > 0 { format!("env {n_env}") } else { "env".to_string() },
        false,
        Msg::EnvChip,
    );
    let persist_on = state.spill && state.capture_limit_bytes > 0;
    let persist_chip = chip("persist".to_string(), persist_on, Msg::PersistChip);
    ...
    View::new(Style {
        ...
        flex_shrink: 0.0,
        align_items: Some(llimphi_ui::llimphi_layout::taffy::AlignItems::FlexEnd),
        gap: Size {
    ...
    .children(vec![input_holder, env_chip, persist_chip])
}
