...
- **Caveat.** Reverb global (un solo set de parámetros a la vez), no convolución por respuesta-al-impulso ni transición suave entre sectores — un cambio de cuarto reajusta los parámetros del tick sin crossfade (la cola en vuelo se reasienta). Sin oclusión real (un sonido en el cuarto vecino no se filtra por la pared; sólo el reverb del cuarto del *oyente* cambia). **A validar al correr** (sin device ni WAD en dev). Fase 4.4+: oclusión pasa-bajos por linedef + crossfade de ambiente entre sectores.
