...
9. **Stack LLM agnóstico de proveedor — completo** (2026-05-26). Cuatro crates nuevos para abrir transformaciones generadas por LLM dentro de pluma sin acoplar a ningún proveedor concreto.
   ...
   - **Pendiente**: ejecutor que combine `pluma-transform-llm` con persistencia automática en `pluma-graph` (hoy el caller mete los `atoms_nuevos` al grafo a mano). El embedding global de la suite (rimay-verbo) y el LLM transparente (pluma-llm) son piezas paralelas: una usa `Provider`, la otra `ChatClient`. Ambas inertes a qué motor concreto las atiende.
