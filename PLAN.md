...
| `sys_actualizar_raiz(hash_manifiesto)` | `wawa-kernel/src/wasm/env.rs` + manifiesto.rs | **hecho 2026-05-28** — vive bajo el nombre `sys_manifiesto_proponer` (`env.rs:1670`, CAPACIDAD 7b, gateada por `PERMISO_RAIZ`). Deserializa el sobre, verifica firma contra el `AGORA_AUTH_RING`, exige que el manifiesto referenciado esté en el grafo local, y re-ancla atómicamente. Validación de tipos WASM de apps nuevas antes de reanclar sigue pendiente. |
