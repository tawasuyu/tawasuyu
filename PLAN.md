...
| Cosmos canvas + app | ~~`01_yachay/cosmos/cosmos-app`~~ (GPUI) | 🚧 MVP (2026-05-25 / extendido 2026-05-27): `cosmos-canvas-llimphi` traduce `Vec<DrawCommand>` de `cosmos-render::compose_wheel` a primitivas vello (Circle/Line/Polygon) + texto vía llimphi-text con glyphs unicode astrológicos (☉♀♈…). `cosmos-app-llimphi` (binario) llama `cosmos_engine::compose_with_options` con un `Chart` sample (Lima, 1990-06-21) — cuerpos calculados por VSOP2013 vía eternal-bridge, casas Placidus, aspectos mayores, dignidades esenciales. Toolbar de 7 toggles de overlays (Transit/Progression/SolarArc/Uranian/Lots/FixedStars/Midpoints) reconstruye el `RenderModel` al click. Segunda fila con selector de armónicos H1/H4/H5/H7/H9 → setea `NatalOptions::harmonic`, el engine corre `apply_harmonic` y anota el título con `· HN`. Status bar muestra ms / capas / overlays / aspectos; en error compose() cae a natal puro o `compute_mock` y surface el mensaje. `cosmos-{tree,panel,theme}` GPUI borrados (huérfanos tras la caída de `cosmos-app`); cuando se necesite shell completo, los recreamos en Llimphi sobre `llimphi-widget-tree`. Pendiente: store de cartas + sidebar tree, form de birth data, AstroCarto + box graphs + research (resto del roadmap Kepler §5). |
