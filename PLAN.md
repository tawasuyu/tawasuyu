...
4. **`nahual-shell-llimphi`** — MVP (2026-05-25): file explorer + text viewer en split fijo. Navegación con teclado (↑↓ Enter ⌫), rueda del mouse, y click; preview de archivos texto ≤256KB. Llimphi-ui ganó `clip` (push_layer/pop_layer con `Mix::Clip`, recorta paint **y** hit-test al rect del nodo) y `on_wheel` (delta normalizado a líneas). Primer widget reusable extraído: `llimphi-widget-list` (lista vertical virtualizada con filas clicables, selección y caption). Sin layout.json/persister/hot-reload/Tabs/Tiled/DatabaseExplorer/ImageViewer/AppBus todavía.
