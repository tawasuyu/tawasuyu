//! Tests del runtime JS (`JsRuntime`) y de las ~142 APIs web del bootstrap.
//! Extraído del `mod tests` inline de `lib.rs` (regla #1: el archivo pasaba de
//! 13k LOC, ~88% tests). Sigue siendo `#[cfg(test)] mod tests;` con acceso a los
//! internos del crate vía `use super::*`. Movido VERBATIM (indentación de 4
//! espacios conservada) para no tocar el contenido de los string literals JS
//! multilínea de los tests.
