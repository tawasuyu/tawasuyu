...
impl JsRuntime {
    ...
    /// desde la creación del runtime). console.log lo redirige a stdout
    /// a través del wrapper de `qjs_std_*` del reactor — captura vía
    /// `fd_write(fd=1, ...)`.
    pub fn stdout(&self) -> String {
        self.stdout.lock().unwrap().clone()
    }

    /// Texto acumulado en `console.error`.
