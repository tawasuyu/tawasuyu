...
pub(crate) fn dispatch_a(p: &str, value: &str) -> Option<DeclKind> {
    match p {
        ...
        // === Fase 7.986-7.1005 — props de nicho (plumb opaco; sentinel = initial) ===
        "shape-padding" => opaque_or_sentinel(value, "none").map(DeclKind::ShapePadding),
        "line-fit-edge" => opaque_or_sentinel(value, "leading").map(DeclKind::LineFitEdge),
        "inline-sizing" => opaque_or_sentinel(value, "normal").map(DeclKind::InlineSizing),
        "box-snap" => opaque_or_sentinel(value, "none").map(DeclKind::BoxSnap),
        "copy-into" => opaque_or_sentinel(value, "none").map(DeclKind::CopyInto),
        "line-stacking" => opaque_or_sentinel(value, "normal").map(DeclKind::LineStacking),
        "line-stacking-ruby" => opaque_or_sentinel(value, "exclude-ruby").map(DeclKind::LineStackingRuby),
        "line-stacking-shift" => opaque_or_sentinel(value, "consider-shifts").map(DeclKind::LineStackingShift),
        "line-stacking-strategy" => opaque_or_sentinel(value, "inline-line-height").map(DeclKind::LineStackingStrategy),
        "inline-box-align" => opaque_or_sentinel(value, "last").map(DeclKind::InlineBoxAlign),
        "alignment-adjust" => opaque_or_sentinel(value, "auto").map(DeclKind::AlignmentAdjust),
        "text-height" => opaque_or_sentinel(value, "auto").map(DeclKind::TextHeight),
        "drop-initial-size" => opaque_or_sentinel(value, "auto").map(DeclKind::DropInitialSize),
        "drop-initial-value" => opaque_or_sentinel(value, "initial").map(DeclKind::DropInitialValue),
        "drop-initial-before-align" => opaque_or_sentinel(value, "caps-height").map(DeclKind::DropInitialBeforeAlign),
        "drop-initial-after-align" => opaque_or_sentinel(value, "baseline").map(DeclKind::DropInitialAfterAlign),
        "drop-initial-before-adjust" => opaque_or_sentinel(value, "before-edge").map(DeclKind::DropInitialBeforeAdjust),
        "drop-initial-after-adjust" => opaque_or_sentinel(value, "after-edge").map(DeclKind::DropInitialAfterAdjust),
        "block-progression" => opaque_or_sentinel(value, "tb").map(DeclKind::BlockProgression),
        "snap-height" => opaque_or_sentinel(value, "none").map(DeclKind::SnapHeight),
