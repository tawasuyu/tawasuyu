...
- **Bloqueado por:** nada. Siguiente: font-weight real cuando llimphi-text lo abra, fetch async / cancelable, `:hover` restyle completo (color/border) + `:focus`/`:active`, `:not(simple)` y `:nth-child(an+b)`, persistencia de la cache entre sesiones, `box-shadow`.
