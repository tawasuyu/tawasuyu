...
- **Bloqueado por:** nada. Siguiente: font-weight real cuando llimphi-text lo abra, fetch async paralelo de assets (CSS/imágenes concurrentes en el worker), `:hover` restyle completo (color/border) + `:focus`/`:active`, `:not(simple)` y `:nth-child(an+b)`, cache con TTL respetando `Cache-Control`, `box-shadow`.
