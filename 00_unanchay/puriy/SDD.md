...
- **Bloqueado por:** nada. Siguiente: font-weight real cuando llimphi-text lo abra, fetch async / cancelable, `:hover`/`:focus` (requiere mouse tracking en el chrome), `:not(simple)` y `:nth-child(an+b)`, persistencia de la cache entre sesiones, `border` + `border-radius` + `box-shadow`, `!important`.
