//! Transporte del daemon de voz, abstraído por plataforma.
//!
//! El encuadre ([`crate::wire`]) es agnóstico del transporte: opera sobre
//! cualquier `AsyncRead + AsyncWrite`. Lo único atado al SO es *cómo* se abre el
//! canal. Por eso este módulo expone una API única por `path` y elige la
//! implementación con `cfg` — idéntico a `rimay-verbo-daemon`:
//!
...
//! - **No-Unix (Windows…)**: TCP en loopback con puerto efímero publicado en un
//!   sidecar `<path>.port`; el cliente lo lee para conectar.
...
mod tcp_impl {
    ...
    /// Listener del daemon ligado a un puerto TCP de loopback, publicado en el
    /// sidecar `<path>.port`.
    pub struct Listener {
        inner: TcpListener,
        port_file: PathBuf,
    ...
    pub async fn connect(path: &Path) -> io::Result<Stream> {
        ...
        let txt = std::fs::read_to_string(&port_file).map_err(|_| {
            io::Error::new(io::ErrorKind::NotFound, "sidecar de puerto voz ausente")
        })?;
        let port: u16 = txt.trim().parse().map_err(|_| {
            io::Error::new(io::ErrorKind::InvalidData, "puerto voz inválido en sidecar")
        })?;
