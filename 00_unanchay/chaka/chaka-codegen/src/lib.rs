//! `chaka-codegen` — emisión de Rust desde el IR del transpilador.
//!
//! Etapa final del pipeline COBOL→Rust: toma el [`Ir`] de `chaka-ir`
//! y produce un fuente Rust (un `String`) que, compilado contra
//! `chaka-runtime`, ejecuta la lógica del programa COBOL original.
//!
...
fn emit_header(em: &mut Emitter) {
    em.line("//! Generado por chaka — transpilador COBOL → Rust.");
...
unreachable_code, clippy::all)]",
    ...
    em.line("    Decimal::parse(s).expect(\"chaka: literal numérico inválido\")");
    em.line("}");
    em.blank();
}
...
/// normalizado por `chaka-ir`. Una tabla (`OCCURS n`) se inicializa
