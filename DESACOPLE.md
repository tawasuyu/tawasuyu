...
| 3 | 🟡 **PARCIAL** `tullpu-app-llimphi` (`ops.rs`, `model.rs`) | (1) kernel de pintura buffer-puro (25 fns) → **`tullpu-paint`** (10 tests). (2) motor de undo/redo (snapshots + cursor + coalescing por etiqueta) → **`tullpu_core::Historial<T>`** (4 tests); el `Model` cambia 3 campos por `hist: Historial<Lienzo>` y `historial.rs` queda en wrappers finos. 251 tests siguen verdes. **Pendiente (no hecho a propósito)**: el document-aggregate (lienzo+almacén + pipeline `aplicar_y_recomponer` + las ~50 ops sobre `&mut Model`) NO se extrajo: a diferencia de dominium, `pruebas.rs` (4.6k líneas) accede a `model.lienzo`/`model.almacen` en cientos de sitios → sería relocación de campos guiada por compilador de muy alto churn/riesgo, no liberación de lógica atrapada (esa ya salió a paint+Historial) | resto: doc-aggregate (diferido) |
