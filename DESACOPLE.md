...
| 4 | 🟡 **PARCIAL** `nakui-ui-llimphi` (`backend.rs`), `nakui-sheet-llimphi` (`pivot.rs`) | `backend.rs` (705 LOC: MemoryStore+EventLog+Executors, WAL/snapshot/recovery, auto-compaction, impl `MetaBackend`) movido a **`nakui-backend`** — ya era UI-agnóstico, sólo mal ubicado; `mod backend` queda como re-export fino (`crate::backend::X` intacto). Pendiente: motor de tabla dinámica `pivot.rs` → `nakui-sheet` core | resto: `pivot.rs` → `nakui-sheet` |
