...
/// FASE 41/42/48 :: anillo de claves publicas del operador local,
/// ESPEJO bit-a-bit del `claves::AGORA_AUTH_RING` del kernel. Cualquier
...
///
/// FASE 48 :: las tres pubkeys son las forjadas por la Boot Trust
/// Ceremony (`wawactl claves forjar`). Si el operador rota una clave,
/// el literal nuevo debe reflejarse en AMBAS sedes (aqui + kernel) y
/// la imagen completa se re-forja.
const AGORA_AUTH_RING: [[u8; 32]; 3] = [
    // Slot 0 :: primaria. Espejo del slot 0 del kernel (Fase 48).
    [
    ...
    // Slot 1 :: dispositivo secundario. Espejo del slot 1 del kernel.
    [
    ...
    // Slot 2 :: llave de recuperacion (cold-storage). Espejo del slot 2
    // del kernel.
    [
