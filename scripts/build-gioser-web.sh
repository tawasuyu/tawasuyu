#!/usr/bin/env bash
# =============================================================================
#  scripts/build-gioser-web.sh — build + deploy a un dir estático.
# -----------------------------------------------------------------------------
...
#    ├── 03_ukupacha/ shared/
#    └── web/gioser-web/{README.md,md/*.md}
#
#  Uso:
#    ./scripts/build-gioser-web.sh                  # → /var/www/gioser
#    ./scripts/build-gioser-web.sh /tmp/gioser      # → /tmp/gioser
#    ./scripts/build-gioser-web.sh --dev /tmp/gw    # build dev, deploy a /tmp/gw
#
...
DEST="${DEST:-/var/www/gioser}"

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
WEB="$REPO/web/gioser-web"
...
# --- 1) cargo build ---
echo "==> cargo build ($PROFILE) -p gioser-web"
cd "$REPO"
if [ "$PROFILE" = release ]; then
  cargo build --target wasm32-unknown-unknown --release -p gioser-web
  WASM="$REPO/target/wasm32-unknown-unknown/release/gioser_web.wasm"
else
  cargo build --target wasm32-unknown-unknown -p gioser-web
  WASM="$REPO/target/wasm32-unknown-unknown/debug/gioser_web.wasm"
fi
